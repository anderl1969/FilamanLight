...
void setupDisplay() {
  if (!display.begin(SSD1306_SWITCHCAPVCC, SCREEN_ADDRESS)) {
    ...
    for (;;)
      ; // Stoppe das Programm, wenn das Display nicht initialisiert werden kann
  }
...
std::vector<String> splitTextIntoLines(const String &text, uint8_t textSize) {
  ...
  for (size_t i = 0; i < words.size(); i++) {
    ...
    display.getTextBounds(testLine, 0, OLED_DATA_START, &x1, &y1, &lineWidth,
                          &h);
...
void oledShowMultilineMessage(const String &message, uint8_t size) {
  ...
  int startY =
      OLED_DATA_START + ((OLED_DATA_END - OLED_DATA_START - totalHeight) / 2);

  uint8_t lineDistance = (lines.size() == 2) ? 5 : 0;
  for (size_t i = 0; i < lines.size(); i++) {
    display.setCursor(oled_center_h(lines[i]),
                      startY + (i * lineHeight) + (i == 1 ? lineDistance : 0));
...
void oledShowIcon(const char *icon) {
  ...
  if (icon == "failed") {
    display.drawBitmap(iconStart, OLED_DATA_START, icon_failed, iconSize,
                       iconSize, WHITE);
  } else if (icon == "success") {
    display.drawBitmap(iconStart, OLED_DATA_START, icon_success, iconSize,
                       iconSize, WHITE);
  } else if (icon == "transfer") {
    display.drawBitmap(iconStart, OLED_DATA_START, icon_transfer, iconSize,
                       iconSize, WHITE);
  } else if (icon == "loading") {
    display.drawBitmap(iconStart, OLED_DATA_START, icon_loading, iconSize,
                       iconSize, WHITE);
  }
...
void oledShowProgressBar(const uint8_t step, const uint8_t numSteps,
                         const char *largeText, const char *statusMessage) {
  ...
  display.drawRoundRect(0, SCREEN_HEIGHT - SCREEN_PROGRESS_BAR_HEIGHT,
                        SCREEN_WIDTH, 12, 6, WHITE);
  display.fillRoundRect(1, SCREEN_HEIGHT - SCREEN_PROGRESS_BAR_HEIGHT + 1,
                        barLength, 10, 6, WHITE);
  display.display();
}
