  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if (error) {
      ...
      if (doc["filament"]["extra"]["nozzle_temperature"].is<String>()) {
        String tempString =
            doc["filament"]["extra"]["nozzle_temperature"].as<String>();
        tempString.replace("[", "");
        tempString.replace("]", "");
      ...
      String cali_idx =
          doc["filament"]["extra"]["bambu_cali_id"].as<String>(); // "\"153\""
      cali_idx.replace("\"", "");

      String bambu_setting_id = doc["filament"]["extra"]["bambu_setting_id"]
                                    .as<String>(); // "\"PFUSf40e9953b40d3d\""
...
void sendToApi(void *parameter) {
  ...
  for (uint8_t attempt = 1; attempt <= MAX_RETRIES && !success; attempt++) {
    ...
    if (octoEnabled && octoToken != "")
      http.addHeader("X-Api-Key", octoToken);

    // Execute HTTP request based on type
    if (httpType == "PATCH")
      httpCode = http.PATCH(updatePayload);
    ...
    if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_CREATED) {
      ...
      if (httpCode >= 400 && httpCode < 500 && httpCode != 408 &&
          httpCode != 429) {
  ...
  if (success) {
    ...
    if (error) {
      ...
      case API_REQUEST_SPOOL_WEIGHT_UPDATE:
        ...
        if (!octoEnabled) {
          ...
          // oledShowProgressBar(1, 1, "Spool Tag", ("Done: " +
          // String(remainingWeight) + " g remain").c_str());
      ...
      case API_REQUEST_OCTO_SPOOL_UPDATE:
        // TBD: Do not use Strings...
        // oledShowProgressBar(5, 5, "Spool Tag", ("Done: " +
        // String(remainingWeight) + " g remain").c_str());
        oledShowMessage("Remaining: " + String(remainingWeight) + "g");
        remainingWeight = 0;
        break;
    ...
    if (triggerWeightUpdate && requestType == API_REQUEST_SPOOL_TAG_ID_UPDATE &&
        weightValue > 10) {
      Serial.println("Executing weight update after successful tag update");

      // Prepare weight update request
      String weightUrl =
          spoolmanUrl + apiUrl + "/spool/" + spoolIdForWeight + "/measure";
      ...
      if (weightHttpCode == HTTP_CODE_OK) {
        ...
        DeserializationError weightError =
            deserializeJson(weightResponseDoc, weightResponse);

        if (!weightError) {
          remainingWeight =
              weightResponseDoc["remaining_weight"].as<uint16_t>();
          ...
          if (!octoEnabled) {
            oledShowProgressBar(
                1, 1, "Spool Tag",
                ("Done: " + String(remainingWeight) + " g remain").c_str());
            remainingWeight = 0;
          } else {
            sendOctoUpdate = true;
          }
    ...
    Serial.println("Fehler beim Senden an Spoolman! HTTP Code: " +
                   String(httpCode));
...
bool updateSpoolTagId(String uidString, const char *payload) {
  ...
  if (params == nullptr) {
    Serial.println(
        "Fehler: Kann Speicher für Task-Parameter nicht allokieren.");
    return false;
  }
  ...
  BaseType_t result =
      xTaskCreate(sendToApi,       // Task-Funktion
...
uint8_t updateSpoolWeight(String spoolId, uint16_t weight) {
  ...
  // Erstelle die Task
  BaseType_t result = xTaskCreate(sendToApi,       // Task-Funktion
                                  "SendToApiTask", // Task-Name
...
bool updateSpoolBambuData(String payload) {
  ...
  String spoolsUrl =
      spoolmanUrl + apiUrl + "/filament/" + doc["filament_id"].as<String>();
  Serial.print("Update Spule mit URL: ");
  Serial.println(spoolsUrl);

  JsonDocument updateDoc;
  updateDoc["extra"]["bambu_setting_id"] =
      "\"" + doc["setting_id"].as<String>() + "\"";
  updateDoc["extra"]["bambu_cali_id"] =
      "\"" + doc["cali_idx"].as<String>() + "\"";
  updateDoc["extra"]["bambu_idx"] =
      "\"" + doc["tray_info_idx"].as<String>() + "\"";
  updateDoc["extra"]["nozzle_temperature"] =
      "[" + doc["temp_min"].as<String>() + "," + doc["temp_max"].as<String>() +
      "]";
...
uint16_t createVendor(const JsonDocument &payload) {
  ...
  // Note: Due to async nature, the ID will be stored in createdVendorId global
  // variable Note: This function assumes that the caller has already ensured
  // API is IDLE
  ...
  if (payload["u"].is<String>()) {
    ...
    // "https://www.blubb.de/f1234/?suche=irgendwas" -> "https://www.blubb.de")
  ...
  // Create task without additional API state check since caller ensures
  // synchronization
  BaseType_t result = xTaskCreate(sendToApi,       // Task-Funktion
                                  "SendToApiTask", // Task-Name
                                  6144,            // Stackgröße in Bytes
...
uint16_t checkVendor(const JsonDocument &payload) {
  ...
  foundVendorId =
      65535; // Reset to invalid value to detect when API response is received
...
uint16_t createFilament(uint16_t vendorId, const JsonDocument &payload) {
  ...
  // Note: Due to async nature, the ID will be stored in createdFilamentId
  // global variable Note: This function assumes that the caller has already
  // ensured API is IDLE
  ...
  filamentDoc["density"] =
      (payload["de"].is<String>() && payload["de"].as<String>().length() > 0)
          ? payload["de"].as<String>()
          : "1.24";
  filamentDoc["diameter"] =
      (payload["di"].is<String>() && payload["di"].as<String>().length() > 0)
          ? payload["di"].as<String>()
          : "1.75";
  ...
  filamentDoc["settings_extruder_temp"] =
      payload["et"].is<String>() ? payload["et"].as<String>() : "";
  filamentDoc["settings_bed_temp"] =
      payload["bt"].is<String>() ? payload["bt"].as<String>() : "";

  if (payload["an"].is<String>()) {
    filamentDoc["external_id"] = payload["an"].as<String>();
    filamentDoc["comment"] =
        payload["u"].is<String>()
            ? payload["u"].as<String>() + payload["an"].as<String>()
            : "automatically generated";
  } else {
    filamentDoc["comment"] = payload["u"].is<String>()
                                 ? payload["u"].as<String>()
                                 : "automatically generated";
  }
  ...
  if (payload["mc"].is<String>()) {
    ...
    filamentDoc["multi_color_direction"] =
        payload["mcd"].is<String>() ? payload["mcd"].as<String>() : "";
  } else {
    filamentDoc["color_hex"] =
        (payload["c"].is<String>() && payload["c"].as<String>().length() >= 6)
            ? payload["c"].as<String>()
            : "FFFFFF";
  }
...
uint16_t checkFilament(uint16_t vendorId, const JsonDocument &payload) {
  ...
  String spoolsUrl = spoolmanUrl + apiUrl +
                     "/filament?vendor.id=" + String(vendorId) +
                     "&external_id=" + String(payload["artnr"].as<String>());
  ...
  // Wait until foundFilamentId is updated by the API response (not 65535
  // anymore)
...
uint16_t createSpool(uint16_t vendorId, uint16_t filamentId,
                     JsonDocument &payload, String uidString) {
  oledShowProgressBar(5, 5, "New Brand", "Create new Spool");

  // Create new spool in Spoolman database using task system
  // Note: Due to async nature, the ID will be stored in createdSpoolId global
  // variable Note: This function assumes that the caller has already ensured
  // API is IDLE
  createdSpoolId =
      65535; // Reset to invalid value to detect when API response is received
  ...
  spoolDoc["initial_weight"] =
      weight > 10 ? String(weight - payload["sw"].as<int>()) : "1000";
  spoolDoc["spool_weight"] =
      (payload["sw"].is<String>() && payload["sw"].as<String>().length() > 0)
          ? payload["sw"].as<String>()
          : "180";
  spoolDoc["remaining_weight"] = spoolDoc["initial_weight"];
  spoolDoc["lot_nr"] =
      (payload["an"].is<String>() && payload["an"].as<String>().length() > 0)
          ? payload["an"].as<String>()
          : "";
  ...
  // Create optimized JSON structure with sm_id at the beginning for fast-path
  // detection
  JsonDocument optimizedPayload;
  optimizedPayload["sm_id"] =
      String(createdSpoolId); // Place sm_id first for fast scanning
...
bool createBrandFilament(JsonDocument &payload, String uidString) {
  ...
  Serial.println("SUCCESS: Brand filament created with Spool ID: " +
                 String(spoolId));
  return true;
}
...
bool checkSpoolmanExtraFields() {
  ...
  if (!spoolmanExtraFieldsChecked) {
    HTTPClient http;
    String checkUrls[] = {spoolmanUrl + apiUrl + "/field/spool",
                          spoolmanUrl + apiUrl + "/field/filament"};

    String spoolExtra[] = {"nfc_id"};

    String filamentExtra[] = {
        "nozzle_temperature", "price_meter", "price_gramm", "bambu_setting_id",
        "bambu_cali_id",      "bambu_idx",   "bambu_k",     "bambu_flow_ratio",
        "bambu_max_volspeed"};

    String spoolExtraFields[] = {"{\"name\": \"NFC ID\","
                                 "\"key\": \"nfc_id\","
                                 "\"field_type\": \"text\"}"};

    String filamentExtraFields[] = {"{\"name\": \"Nozzle Temp\","
                                    "\"unit\": \"°C\","
...
bool checkSpoolmanInstance() {
  ...
  // Only do the spoolman instance check if there is no active API request going
  // on
  if (spoolmanApiState == API_IDLE) {
    ...
    if (httpCode > 0) {
      ...
      spoolmanConnected = false;
      Serial.println("Error contacting spoolman instance! HTTP Code: " +
                     String(httpCode));
    }
    http.end();
    spoolmanApiState = API_IDLE;
...
bool initSpoolman() {
  oledShowProgressBar(3, 5, DISPLAY_BOOT_TEXT, "Spoolman init");
