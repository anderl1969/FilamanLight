 ...
 */
bool intervalElapsed(unsigned long currentTime, unsigned long &lastTime,
...
void loop() {
  ...
  if (touchSensorConnected && digitalRead(TTP223_PIN) == HIGH &&
  ...
  if (intervalElapsed(currentMillis, lastTopRowUpdateTime,
                      DISPLAY_UPDATE_INTERVAL)) {
    oledShowTopRow();
  }

  // Periodic spoolman health check
  if (intervalElapsed(currentMillis, lastSpoolmanHealcheckTime,
                      SPOOLMAN_HEALTHCHECK_INTERVAL)) {
  ...
  if (bambuCredentials.autosend_enable && autoSetToBambuSpoolId > 0 &&
    ...
    if (intervalElapsed(currentMillis, lastAutoSetBambuAmsTime,
                        autoSetBambuAmsInterval)) {
      if (nfcReaderState == NFC_IDLE) {
        lastAutoSetBambuAmsTime = currentMillis;
        oledShowMessage(
            "Auto Set         " +
            String(bambuCredentials.autosend_time - autoAmsCounter) + "s");
        autoAmsCounter++;
  ...
  //     // Use filtered weight for smooth display, but still check API weight
  //     for
  //     // significant changes
  //     int16_t displayWeight = getFilteredDisplayWeight();
  //     if (mainTaskWasPaused ||
  //         (weight != lastWeight && nfcReaderState == NFC_IDLE &&
  //          (!bambuCredentials.autosend_enable || autoSetToBambuSpoolId ==
  //          0))) {
  //       (displayWeight < 2) ? ((displayWeight < -2) ? oledShowMessage("!!
  //       -0")
  //                                                   : oledShowWeight(0))
  //                           : oledShowWeight(displayWeight);
  //     }
  ...
  //   if (currentMillis - lastWeightReadTime >= weightReadInterval &&
  //       nfcReaderState != NFC_IDLE && nfcReaderState != NFC_READ_SUCCESS) {
  //     weightCounterToApi = 0;
  //   }

  //   lastWeight = weight;

  //   // Wenn ein Tag mit SM id erkannte wurde und der Waage Counter anspricht
  //   an
  //   // SM Senden
  //   if (activeSpoolId != "" && weightCounterToApi > 3 && weightSend == 0 &&
  //       nfcReaderState == NFC_READ_SUCCESS && tagProcessed == false &&
  //       spoolmanApiState == API_IDLE) {
  //     // set the current tag as processed to prevent it beeing processed
  //     again tagProcessed = true;
  ...
  //   // Handle successful tag write: Send weight to Spoolman but NEVER
  //   auto-send
  //   // to Bambu
  //   if (activeSpoolId != "" && weightCounterToApi > 3 && weightSend == 0 &&
  //       nfcReaderState == NFC_WRITE_SUCCESS && tagProcessed == false &&
  //       spoolmanApiState == API_IDLE) {
  //     // set the current tag as processed to prevent it beeing processed
  //     again tagProcessed = true;
  ...
  //       weightSend = 1;
  //       Serial.println(
  //           "Tag written: Weight sent to Spoolman, but NO auto-send to
  //           Bambu");
  //       // INTENTIONALLY do NOT set autoSetToBambuSpoolId here to prevent
  //       Bambu
  //       // auto-send
  //     } else {
  //       oledShowIcon("failed");
