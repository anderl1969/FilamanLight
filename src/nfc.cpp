...
volatile bool nfcWriteInProgress =
    false; // Prevent any tag operations during write
...
void payloadToJson(uint8_t *data) {
  ...
  if (startJson && endJson && endJson > startJson) {
    ...
    Serial.println(
        "Kein gültiger JSON-Inhalt gefunden oder fehlerhafte Formatierung.");
...
String detectNtagType() {
  ...
  // Try to read page 130 (NTAG215 ends at page 129, so this should fail for
  // NTAG213/215)
  ...
  if (dataAreaSize <= 180 && !canReadPage41) {
    ...
    tagType = "NTAG215";
    Serial.println(
        "Detected: NTAG215 (can read page 41, cannot read page 130)");
...
bool clearUserDataArea() {
  ...
  // NTAG layout: Pages 0-3 (header), 4-N (user data), N+1-N+3 (config) - NEVER
  // touch config!
  String tagType = detectNtagType();
...
uint8_t ntag2xx_WriteNDEF(const char *payload) {
  ...
  Serial.print("Total Tag Size: ");
  Serial.println(tagSize);
  Serial.print("Available User Data: ");
  Serial.println(availableUserData);
  Serial.print("Max Writable Page: ");
  ...
  if (!nfc.ntag2xx_ReadPage(maxWritablePage, testBuffer)) {
    ...
    // Find actual maximum writable page by testing backwards with optimized
    // approach
    ...
    Serial.print(maxWritablePage);
    Serial.println(" is readable");
  }

  // Calculate maximum available user data based on actual writable pages
  uint16_t actualUserDataSize =
      (maxWritablePage - 3) * 4; // -3 because pages 0-3 are header
  ...
  uint8_t ndefRecordHeaderSize =
      3; // Header byte + Type Length + Payload Length (short record)
  ...
  if (totalTlvSize > availableUserData) {
    ...
    Serial.print("Tag-Typ: ");
    Serial.println(tagType);
    Serial.print("Benötigt: ");
    Serial.print(totalTlvSize);
    Serial.println(" Bytes");
    Serial.print("Verfügbar: ");
    Serial.print(availableUserData);
    Serial.println(" Bytes");
    Serial.print("Überschuss: ");
    Serial.print(totalTlvSize - availableUserData);
    Serial.println(" Bytes");

    if (tagType == "NTAG213") {
      Serial.println("EMPFEHLUNG: Verwenden Sie einen NTAG215 (504 Bytes) oder "
                     "NTAG216 (888 Bytes) Tag!");
      Serial.println("Oder kürzen Sie die Payload um mindestens " +
                     String(totalTlvSize - availableUserData) + " Bytes.");
    }
    Serial.println("!!!!!!!!!!!!!!!!!!!!!!!!");
    Serial.println();
  ...
  Serial.println(
      "✓ Payload passt in den Tag - Schreibvorgang wird fortgesetzt");

  // STEP 1: NFC Interface Reset and Reinitialization
  Serial.println();
  Serial.println(
      "=== SCHRITT 1: NFC-INTERFACE RESET UND NEUINITIALISIERUNG ===");
  ...
  if (!ccReadable) {
    Serial.println(
        "❌ NFC-Interface ist nicht funktionsfähig - führe Reset durch");
    ...
    if (!ccReadable) {
      Serial.println(
          "❌ NFC-Interface funktioniert nach Reset immer noch nicht");
  ...
  if (!basicPagesReadable) {
    Serial.println(
        "❌ KRITISCHER FEHLER: Grundlegende NDEF-Seiten nicht lesbar!");
  ...
  if (!nfc.ntag2xx_WritePage(10, testPage)) {
    ...
    if (!tagStillPresent) {
      ...
      Serial.println(
          "Möglicherweise: Schreibschutz, Defekt, oder Interface-Problem");
  ...
  if (!testSuccess) {
    Serial.println(
        "FEHLER: Schreibtest fehlgeschlagen - Daten stimmen nicht überein!");
    Serial.print("Geschrieben: ");
    for (int i = 0; i < 4; i++) {
      Serial.print(testPage[i], HEX);
      Serial.print(" ");
    }
    Serial.println();
  ...
  if (!nfc.ntag2xx_WritePage(10, originalPage)) {
    Serial.println(
        "WARNUNG: Konnte ursprünglichen Inhalt nicht wiederherstellen!");
  } else {
  ...
  if (!interfaceStable) {
    Serial.println(
        "FEHLER: NFC-Interface ist nicht stabil genug für Schreibvorgang");
  ...
  tlvData[offset++] =
      (uint8_t)payloadLen; // Payload Length (short record format)
  ...
  while (bytesWritten < totalBytes && pageNumber <= maxWritablePage) {
    ...
    vTaskDelay(
        10 /
        portTICK_PERIOD_MS); // Slightly increased delay between page writes
  }
  ...
  Serial.print(bytesWritten);
  Serial.println(" Bytes geschrieben");
  Serial.print("✓ Verwendete Seiten: 4-");
  Serial.println(pageNumber - 1);
  ...
  Serial.println();
  Serial.println(
      "=== SCHRITT 7: NFC-INTERFACE STABILISIERUNG NACH SCHREIBVORGANG ===");
  ...
  for (int stabilityAttempt = 0; stabilityAttempt < 5; stabilityAttempt++) {
    ...
    if (nfc.ntag2xx_ReadPage(3, postWriteTest)) { // Read capability container
      ...
      if (stabilityAttempt < 4) {
        ...
        uint8_t uid[] = {0, 0, 0, 0, 0, 0, 0};
        uint8_t uidLength;
        bool tagStillPresent = nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A,
                                                       uid, &uidLength, 1000);
  ...
  if (!interfaceResponsive) {
    Serial.println("WARNUNG: NFC-Interface reagiert nach Schreibvorgang nicht "
                   "mehr stabil");
    Serial.println("Schreibvorgang war erfolgreich, aber Interface benötigt "
                   "möglicherweise Reset");
  } else {
...
bool decodeNdefAndReturnJson(const byte *encodedMessage, String uidString) {
  ...
  if (encodedMessage[tlvOffset + 1] == 0xFF) {
    ...
    ndefMessageLength =
        (encodedMessage[tlvOffset + 2] << 8) | encodedMessage[tlvOffset + 3];
  ...
  for (uint32_t i = 0; i < payloadLength; i++) {
    ...
    if (currentByte == '}') {
      ...
      for (uint32_t j = 0; j <= i; j++) {
        if (ndefRecord[payloadOffset + j] == '{')
          braceCount++;
        else if (ndefRecord[payloadOffset + j] == '}')
          braceCount--;
      }
    ...
    if (spoolmanConnected) {
      ...
      if (doc["sm_id"].is<String>() && doc["sm_id"] != "" &&
        ...
        if (lastSpoolId != "") {
          ...
          Serial.println(
              "Location update tag scanned without scanning spool before!");
          oledShowProgressBar(1, 1, "Failure", "Scan spool first");
        }
      }
      // Brand Filament not registered to Spoolman
      else if ((!doc["sm_id"].is<String>() ||
                (doc["sm_id"].is<String>() &&
                 (doc["sm_id"] == "0" || doc["sm_id"] == ""))) &&
      ...
      oledShowProgressBar(octoEnabled ? 5 : 4, octoEnabled ? 5 : 4, "Failure!",
...
bool readCompleteJsonForFastPath() {
  ...
  bool success =
      decodeNdefAndReturnJson(data, ""); // Empty UID string for fast-path
...
bool quickSpoolIdCheck(String uidString) {
  ...
  // Read enough pages to cover NDEF header + beginning of payload (pages 4-8 =
  // 20 bytes)
  uint8_t ndefData[20];
  ...
  uint8_t idLength =
      (recordHeader & 0x08)
          ? ndefData[ndefRecordStart + 2 + payloadLengthBytes + typeLength]
          : 0; // IL flag check

  int payloadOffset =
      ndefRecordStart + 1 + 1 + payloadLengthBytes + typeLength + idLength;
  ...
  if (payloadOffset >= 20) {
    Serial.println("✗ FAST-PATH: JSON payload starts beyond quick read data - "
                   "need more pages");
    ...
    if (jsonStart.indexOf("\"sm_id\":\"") >= 0) {
      ...
      if (smIdEnd > smIdStart && smIdEnd < jsonStart.length()) {
        ...
        if (quickSpoolId != "0" && quickSpoolId.length() > 0) {
          ...
          Serial.println(
              "FAST-PATH: Reading complete JSON data for web interface...");
          if (readCompleteJsonForFastPath()) {
            Serial.println(
                "✓ FAST-PATH: Complete JSON data loaded for web interface");
          } else {
            Serial.println("⚠ FAST-PATH: Could not read complete JSON, web "
                           "interface may show limited data");
          }

          oledShowProgressBar(2, octoEnabled ? 5 : 4, "Known Spool",
                              "Quick mode");
          Serial.println("✓ FAST-PATH SUCCESS: Known spool processed quickly");
          return true;
        } else {
          Serial.println(
              "✗ FAST-PATH: sm_id is 0 - new brand filament, need full read");
          return false;
        }
      }
    }
  ...
  // Look for sm_id pattern in the beginning of JSON - check for known vs new
  // spools
  if (quickJson.indexOf("\"sm_id\":\"") >= 0) {
    ...
    if (smIdEnd > smIdStart && smIdEnd < quickJson.length()) {
      ...
      if (quickSpoolId != "0" && quickSpoolId.length() > 0) {
        ...
        Serial.println(
            "✗ FAST-PATH: sm_id is 0 - new brand filament, need full read");
        return false; // sm_id="0" means new brand filament, needs full
                      // processing
      }
    } else {
  ...
  if (quickJson.indexOf("\"brand\":\"") >= 0) {
    Serial.println(
        "✓ FAST-PATH: Brand filament detected - may need full processing");
    return false; // Need full read for brand filament creation
  }

  Serial.println(
      "✗ FAST-PATH: No recognizable pattern - falling back to full read");
...
void writeJsonToTag(void *parameter) {
  ...
  Serial.println("NFC Write Task starting - High-level operations blocked, "
                 "low-level NFC available");
  ...
  for (uint16_t i = 0; i < 20; i++) {
    ...
    esp_task_wdt_reset();
    success =
        nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength, 400);
    if (success) {
    ...
    if (success) {
      ...
      if (params->tagType) {
        ...
        if (updateSpoolTagId(uidString, params->payload) && params->tagType) {
          ...
          if (weight > 20) {
            Serial.println("Tag successfully written and weight > 20g - "
                           "sending weight to Spoolman");

            // Extract spool ID from payload for weight update
            JsonDocument payloadDoc;
            DeserializationError error =
                deserializeJson(payloadDoc, params->payload);
            ...
            Serial.printf("Weight %dg is not above 20g threshold - skipping "
                          "weight update\n",
                          weight);
          }
        } else {
      ...
      while (tagRemovalChecks < 10) {
        ...
        bool tagPresent = nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid,
                                                  &uidLength, 500);
      ...
      if (tagRemovalChecks >= 10) {
        Serial.println(
            "WARNUNG: Tag wurde nicht entfernt - fahre trotzdem fort");
      }
      ...
      if (!interfaceReady) {
        Serial.println("WARNUNG: NFC-Interface reagiert nicht - könnte normale "
                       "Scans beeinträchtigen");
      } else {
...
void startWriteJsonToTag(const bool isSpoolTag, const char *payload) {
  ...
  parameters->payload =
      strdup(optimizedPayload.c_str()); // Use optimized payload

  // Task nicht mehrfach starten
  if (nfcReaderState == NFC_IDLE || nfcReaderState == NFC_READ_ERROR ||
      nfcReaderState == NFC_READ_SUCCESS) {
    oledShowProgressBar(0, 1, "Write Tag", "Place tag now");
    // Erstelle die Task
    xTaskCreate(writeJsonToTag,       // Task-Funktion
...
void scanRfidTask(void *parameter) {
  ...
  for (;;) {
    ...
    if (nfcReaderState != NFC_WRITING && !nfcWriteInProgress &&
        !nfcReadingTaskSuspendRequest && !booting) {
      ...
      // Reset activeSpoolId immediately when no tag is detected to prevent
      // stale autoSet
      if (!success) {
        activeSpoolId = "";
      }

      // As long as there is still a tag on the reader, do not try to read it
      // again
      if (success && nfcReaderState == NFC_IDLE) {
        ...
        if (uidLength == 7) {
          ...
          if (quickSpoolIdCheck(uidString)) {
            Serial.println(
                "✓ FAST-PATH: Tag processed quickly, skipping full read");
            pauseBambuMqttTask = false;
          ...
          if (tagSize > 0) {
            ...
            // We probably have an NTAG2xx card (though it could be Ultralight
            // as well)
            ...
            for (uint8_t i = 4; i < 4 + numPages; i++) {
              ...
              if (!robustPageRead(i, data + (i - 4) * 4)) {
                Serial.printf(
                    "Failed to read page %d after retries, stopping\n", i);
            ...
            activeSpoolId = "";
            Serial.println(
                "Tag read failed - activeSpoolId reset to prevent autoSet");
          }
        } else {
          // TBD: Show error here?!
          oledShowProgressBar(1, 1, "Failure", "Unkown tag type");
          Serial.println("This doesn't seem to be an NTAG2xx tag (UUID length "
                         "!= 7 bytes)!");
          // Reset activeSpoolId when tag type is unknown to prevent autoSet
          activeSpoolId = "";
          Serial.println(
              "Unknown tag type - activeSpoolId reset to prevent autoSet");
        }
      }

      if (!success && nfcReaderState != NFC_IDLE &&
          !nfcReadingTaskSuspendRequest) {
        nfcReaderState = NFC_IDLE;
        // uidString = "";
        nfcJsonData = "";
        activeSpoolId = "";
      ...
      // Reset state after successful read when tag is removed
        ...
        nfcReaderState = NFC_IDLE;
        Serial.println(
            "Tag nach erfolgreichem Lesen entfernt - bereit für nächsten Tag");
      }

      // Add a pause after successful reading to prevent immediate re-reading
      if (nfcReaderState == NFC_READ_SUCCESS) {
        Serial.println(
            "Tag erfolgreich gelesen - warte 3 Sekunden vor nächstem Scan");
        vTaskDelay(3000 /
                   portTICK_PERIOD_MS); // Reduced from 5 seconds to 3 seconds
      } else {
...
void startNfc() {
  oledShowProgressBar(5, 5, DISPLAY_BOOT_TEXT, "NFC init");
  nfc.begin(); // Beginne Kommunikation mit RFID Leser
  delay(1000);
  unsigned long versiondata =
      nfc.getFirmwareVersion(); // Lese Versionsnummer der Firmware aus
  if (!versiondata) {           // Wenn keine Antwort kommt
    Serial.println(
        "Kann kein RFID Board finden !"); // Sende Text "Kann kein..." an
                                          // seriellen Monitor
    oledShowMessage("No RFID Board found");
    vTaskDelay(2000 / portTICK_PERIOD_MS);
  } else {
    Serial.print("Chip PN5 gefunden");
    Serial.println((versiondata >> 24) & 0xFF,
                   HEX); // Sende Text und Versionsinfos an seriellen
    Serial.print("Firmware ver. ");
    Serial.print((versiondata >> 16) & 0xFF,
                 DEC); // Monitor, wenn Antwort vom Board kommt
    Serial.print('.');
