...
void onWsEvent(AsyncWebSocket *server, AsyncWebSocketClient *client,
  ...
  if (type == WS_EVT_CONNECT) {
    ...
    Serial.println("Currently connected number of clients: " +
                   String((*server).getClients().size()));
    ...
    if (doc["type"] == "heartbeat") {
      ...
      ws.text(client->id(), "{"
                            ...
                            "\"freeHeap\":" +
                                ...
                                ","
                                "\"bambu_connected\":" +
                                String(bambu_connected) +
                                ","
                                "\"spoolman_connected\":" +
                                String(spoolmanConnected) +
                                ""
                                "}");
    }
      ...
      if (doc["payload"].is<JsonObject>()) {
        ...
        startWriteJsonToTag((doc["tagType"] == "spool") ? true : false,
                            payloadString.c_str());
      }
    }
      ...
      if (updateSpoolBambuData(doc["payload"].as<String>())) {
        ws.textAll(
            "{\"type\":\"setSpoolmanSettings\",\"payload\":\"success\"}");
      } else {
...
void sendWriteResult(AsyncWebSocketClient *client, uint8_t success) {
  ...
  String response =
      "{\"type\":\"writeNfcTag\",\"success\":" + String(success ? "1" : "0") +
      "}";
  ws.textAll(response);
}
...
void foundNfcTag(AsyncWebSocketClient *client, uint8_t success) {
  if (success == lastSuccess)
    return;
  ws.textAll("{\"type\":\"nfcTag\", \"payload\":{\"found\": " +
             String(success) + "}}");
  sendNfcData();
  lastSuccess = success;
}
...
void sendNfcData() {
  ...
  case NFC_READ_SUCCESS:
    ...
    break;
  case NFC_READ_ERROR:
    ws.textAll("{\"type\":\"nfcData\", \"payload\":{\"error\":\"Empty Tag or "
               "Data not readable\"}}");
    break;
  case NFC_WRITING:
    ws.textAll(
        "{\"type\":\"nfcData\", \"payload\":{\"info\":\"Schreibe Tag...\"}}");
    break;
  case NFC_WRITE_SUCCESS:
    ws.textAll("{\"type\":\"nfcData\", \"payload\":{\"info\":\"Tag erfolgreich "
               "geschrieben\"}}");
    break;
  case NFC_WRITE_ERROR:
    ws.textAll("{\"type\":\"nfcData\", \"payload\":{\"error\":\"Error writing "
               "to Tag\"}}");
    break;
  case DEFAULT:
    ws.textAll("{\"type\":\"nfcData\", \"payload\":{\"error\":\"Something went "
               "wrong\"}}");
  }
...
void setupWebserver(AsyncWebServer &server) {
  oledShowProgressBar(2, 5, DISPLAY_BOOT_TEXT, "Webserver init");
  ...
  server.on("/waage", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    // "/waage.html.gz", "text/html"); response->addHeader("Content-Encoding",
    // "gzip"); response->addHeader("Cache-Control", CACHE_CONTROL);
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    AsyncWebServerResponse *response =
        request->beginResponse(LittleFS, page, "text/html");
  ...
  server.on("/wifi", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    AsyncWebServerResponse *response =
        request->beginResponse(LittleFS, "/wifi.html.gz", "text/html");
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Cache-Control", CACHE_CONTROL);
    request->send(response);
  });
  ...
  server.on("/spoolman", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    html.replace("{{bambuCode}}", bambuCredentials.accesscode
                                      ? bambuCredentials.accesscode
                                      : "");
    html.replace("{{autoSendToBambu}}",
                 bambuCredentials.autosend_enable ? "checked" : "");
    html.replace("{{autoSendTime}}",
                 (bambuCredentials.autosend_time != 0)
                     ? String(bambuCredentials.autosend_time)
                     : String(BAMBU_DEFAULT_AUTOSEND_TIME));
  ...
  server.on("/style.css", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    AsyncWebServerResponse *response =
        request->beginResponse(LittleFS, "/style.css.gz", "text/css");
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Cache-Control", CACHE_CONTROL);
    request->send(response);
  ...
  server.on("/logo.png", HTTP_GET, [](AsyncWebServerRequest *request) {
    AsyncWebServerResponse *response =
        request->beginResponse(LittleFS, "/logo.png.gz", "image/png");
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Cache-Control", CACHE_CONTROL);
    request->send(response);
  ...
  server.on("/favicon.ico", HTTP_GET, [](AsyncWebServerRequest *request) {
    AsyncWebServerResponse *response =
        request->beginResponse(LittleFS, "/favicon.ico", "image/x-icon");
    response->addHeader("Cache-Control", CACHE_CONTROL);
    request->send(response);
  ...
  server.on("/spool_in.png", HTTP_GET, [](AsyncWebServerRequest *request) {
    AsyncWebServerResponse *response =
        request->beginResponse(LittleFS, "/spool_in.png.gz", "image/png");
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Cache-Control", CACHE_CONTROL);
    request->send(response);
  ...
  server.on("/set_spoolman.png", HTTP_GET, [](AsyncWebServerRequest *request) {
    AsyncWebServerResponse *response =
        request->beginResponse(LittleFS, "/set_spoolman.png.gz", "image/png");
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Cache-Control", CACHE_CONTROL);
    request->send(response);
  ...
  server.on("/spoolman.js", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    AsyncWebServerResponse *response =
        request->beginResponse(LittleFS, "/spoolman.js.gz", "text/javascript");
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Cache-Control", CACHE_CONTROL);
    request->send(response);
  ...
  server.on("/rfid.js", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    AsyncWebServerResponse *response =
        request->beginResponse(LittleFS, "/rfid.js.gz", "text/javascript");
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Cache-Control", CACHE_CONTROL);
    request->send(response);
  ...
  server.on("/upgrade", HTTP_GET, [](AsyncWebServerRequest *request) {
    AsyncWebServerResponse *response =
        request->beginResponse(LittleFS, "/upgrade.html.gz", "text/html");
    response->addHeader("Content-Encoding", "gzip");
